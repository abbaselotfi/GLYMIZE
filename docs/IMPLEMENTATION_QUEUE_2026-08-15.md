...
5. Complete release acceptance: full-report OCR extraction when tables collapse to one line, review-first OCR extraction of patient-header identity/basic demographics (name/full name, national ID, reported age, reported sex, height, weight) with provenance and no silent overwrite, Persian Unicode Presentation-Form/tatweel/Yeh-Kaf normalization and RTL value-before-label tolerance, rendered-header fallback for corrupted RTL PDF text layers, and explicit laboratory/report date propagation to extracted observations, actionable/deduplicated multipart-name review, unit-adjacent quantitative value disambiguation, low-confidence row highlighting, and protection against H:High/L:Low legend contamination, RC same-origin clinical-runtime gateway so physician/assistant flows do not depend on direct `workers.dev` reachability, sequential Care Team `Create new patient handoff` with top quick access and unsaved-draft save/discard/cancel protection, fail-closed duplicate patient-code creation guard with explicit create/update intent, optimistic revision checks and advisory first-free numeric file-number suggestion, Care Team creation UI limited to file number/national ID, permission changes, CSV export, synthetic user deletion/login failure, post-deploy smoke, production invariant.
6. Patient Record v2 runtime adapter is the current slice. Schema/contracts and the isolated RC migration rehearsal for `0001`–`0003` are complete; Production is unchanged and applied migration `0003` is frozen. Implement practice-scoped smart resolve/create/identifier-attach, monotonic allocator, independent encounter creation/snapshot/observation persistence and a lightweight Workspace read model while keeping legacy handoff compatibility explicit. Do not cut Care Team UI over until optimistic draft snapshot revision semantics prevent repeated saves from creating duplicate encounters.
...
11. Patient Workspace: identifier search, patient header, expandable encounter timeline, pre-visit medication reconciliation vs post-visit signed orders, mini/expanded verified trend charts with pinned metrics/event annotations, pending/completed investigation orders, and deterministic What changed since last visit.

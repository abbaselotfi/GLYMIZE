...
export function enrichCandidateWithDoseMarketCostV2(
  ...
  if (request.preferences.costPreference === "insured_only" && result.insuranceFit === "not_covered" && result.gate.status !== "exclude") {
    if (result.components.some((component) => /insulin/.test(component.therapyGroup)) && result.preferenceConflicts.some((item) => item.includes("الزام بالینی"))) {
      result.preferenceConflicts.push("بیمه انتخاب‌شده این رژیم را پوشش نمی‌دهد، اما الزام بالینی مانع حذف کورکورانه آن شده است.");
    } else {
      result.gate.status = "exclude";
      result.gate.reasons.push("insured-only فعال است و پوشش قابل استفاده برای این رژیم یافت نشد.");
    }
  }

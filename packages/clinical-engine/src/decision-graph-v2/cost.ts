...
export function calculateProductMonthlyCostV2(input: {
  ...
  // GLYMIZE_COST_INPUT_GUARD_V5
  // Costing is a safety boundary: malformed market/dose data must fail closed.
  const priceToman = product.priceToman;
  const purchaseUnitSize = product.consumptionUnitsPerPurchaseUnit;

  if (
    priceToman === undefined ||
    !Number.isFinite(priceToman) ||
    priceToman <= 0 ||
    !Number.isFinite(purchaseUnitSize) ||
    purchaseUnitSize <= 0
  ) {
    return undefined;
  }
  ...
  if (
    product.consumptionUnitsPerContainer !== undefined &&
    (
      !Number.isFinite(product.consumptionUnitsPerContainer) ||
      product.consumptionUnitsPerContainer <= 0
    )
  ) {
    return undefined;
  }
  ...
  const pricePerConsumptionUnit = priceToman / purchaseUnitSize;
  ...
  if (
    derivedNumericValues.some((value) => !Number.isFinite(value) || value < 0)
  ) {
    return undefined;
  }

  // GLYMIZE_COST_DERIVED_GUARD_V5
  return {
    ...
    cashPurchaseCostToman: Math.round(purchaseUnits * priceToman),

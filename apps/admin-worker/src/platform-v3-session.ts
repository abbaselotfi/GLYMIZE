import { openAuthPayload, randomToken, sealAuthPayload, sha256Hex } from "./runtime-security";
...
export async function v3IssueSession(env:V3Env,user:V3User,rememberMe:boolean,deviceLabel?:string){
 ...
 await v3db(env).prepare(`INSERT INTO refresh_tokens(id,user_id,practice_id,token_hash,persistent,expires_at,revoked_at,created_at,last_used_at,device_label,family_id,parent_token_id) VALUES(?,?,?,?,?,?,NULL,?,?,?,?,NULL)`).bind(sessionId,user.id,user.practiceId,refreshHash,rememberMe?1:0,refreshExpiresAt,v3now(),v3now(),String(deviceLabel??"").slice(0,180)||null,sessionId).run();
 const expiresAt=Date.now()+20*60*1000,sealed=await sealAuthPayload({kind:"runtime_access",userId:user.id,practiceId:user.practiceId,sessionId,expiresAt} satisfies Access,env,"RUNTIME-ACCESS-V1");
...
export async function v3RequireRuntime(request:Request,env:V3Env){
 ...
 const access=await openAuthPayload<Access>({iv,ciphertext},env,"RUNTIME-ACCESS-V1");if(!access||access.kind!=="runtime_access"||access.expiresAt<=Date.now())return null;

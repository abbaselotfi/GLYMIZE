...
/**
 * Legacy patient_handoffs is read-only after the Patient Record v2 cutover.
 * Lookup/list/get remain temporarily available only for unpromoted records.
 * New writes and code-allocation checks belong to Patient Record v2.
...
async function platformRoute(request:Request,env:Env):Promise<Response|null> {
  ...
  if (
    (
      url.pathname === "/v1/patient-handoff/upsert" ||
      url.pathname === "/v1/patient-handoff/code-status"
    ) &&
    request.method === "POST"
  ) {
    return json(
      ...
      env,
      {
        error: "LEGACY_HANDOFF_WRITE_RETIRED",
        replacement: "patient_record_v2",
      },
      410,
    );
  }

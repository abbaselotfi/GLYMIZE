...
const EMPTY_CAPS:RuntimeV3Capabilities={passwordLogin:false,passwordSetup:false,adminUsers:false};
...
export default function AccountV3Client(){
 ...
 const [medicalCouncilCode,setMedicalCouncilCode]=useState(""),[firstName,setFirstName]=useState(""),[lastName,setLastName]=useState(""),[email,setEmail]=useState(""),[mobile,setMobile]=useState("");
 ...
 return <main className={styles.page} dir={isRtl?"rtl":"ltr"}>
  <section className={styles.hero}><span>GLYMIZE IDENTITY</span><h1>{fa?"هویت حرفه‌ای و ورود امن":"Professional identity & secure sign-in"}</h1><p>{fa?"پزشک پس از احراز نظام پزشکی با کد نظام پزشکی و رمز عبور شخصی وارد می‌شود. حساب مدیریت همچنان از مسیر امن GitHub Admin جداست.":"After Medical Council verification, physicians sign in with their Medical Council code and personal password. System administration remains separate under GitHub Admin."}</p></section>
  <div className={styles.tabs}><button data-active={mode==="login"} onClick={()=>setMode("login")}>{fa?"ورود پزشک":"Physician sign in"}</button><button data-active={mode==="signup"} onClick={()=>setMode("signup")}>{fa?"ثبت‌نام پزشک":"Physician sign-up"}</button>{adminSessionPresent&&<button data-active={mode==="bootstrap"} onClick={()=>setMode("bootstrap")}>{fa?"راه‌اندازی مالک":"Owner bootstrap"}</button>}</div>
  {!capsChecked?<section className={styles.card}><p>{fa?"در حال بررسی قابلیت‌های ورود امن…":"Checking secure sign-in capabilities…"}</p></section>:mode==="login"?<section className={styles.card}><h2>{fa?"ورود با کد نظام پزشکی":"Sign in with Medical Council code"}</h2>{!passwordReady?<p className={styles.warning}>{readinessMessage}</p>:<form onSubmit={login}><label><span>{fa?"کد نظام پزشکی":"Medical Council code"}</span><input autoComplete="username" inputMode="numeric" value={medicalCouncilCode} onChange={e=>setMedicalCouncilCode(e.target.value.replace(/\D/g,""))}/></label><label><span>{fa?"رمز عبور":"Password"}</span><input autoComplete="current-password" type="password" value={password} onChange={e=>setPassword(e.target.value)}/></label><label className={styles.remember}><input type="checkbox" checked={rememberMe} onChange={e=>setRememberMe(e.target.checked)}/><span>{fa?"ورود من روی این دستگاه حفظ شود":"Keep me signed in on this device"}</span></label><button disabled={busy||!medicalCouncilCode.trim()||!passwordValid}>{busy?(fa?"در حال ورود…":"Signing in…"):(fa?"ورود امن":"Secure sign in")}</button></form>}</section>:<section className={styles.card}><h2>{mode==="bootstrap"?(fa?"ساخت حساب پزشک مالک — فقط مدیریت":"Owner physician bootstrap — admin only"):(fa?"ثبت‌نام پزشک و تعریف رمز":"Physician registration & password")}</h2>{!passwordReady?<p className={styles.warning}>{readinessMessage}</p>:<form onSubmit={mode==="bootstrap"?bootstrap:signup}><div className={styles.grid2}><label><span>{fa?"کد نظام پزشکی":"Medical Council code"}</span><input inputMode="numeric" value={medicalCouncilCode} onChange={e=>setMedicalCouncilCode(e.target.value.replace(/\D/g,""))}/></label><label><span>{fa?"ایمیل":"Email"}</span><input type="email" value={email} onChange={e=>setEmail(e.target.value)}/></label><label><span>{fa?"نام":"First name"}</span><input value={firstName} onChange={e=>setFirstName(e.target.value)}/></label><label><span>{fa?"نام خانوادگی":"Last name"}</span><input value={lastName} onChange={e=>setLastName(e.target.value)}/></label><label><span>{fa?"موبایل":"Mobile"}</span><input inputMode="tel" value={mobile} onChange={e=>setMobile(e.target.value)}/></label><span></span><label><span>{fa?"رمز عبور (حداقل ۱۰ کاراکتر)":"Password (minimum 10 characters)"}</span><input autoComplete="new-password" type="password" value={password} onChange={e=>setPassword(e.target.value)}/></label><label><span>{fa?"تکرار رمز عبور":"Confirm password"}</span><input autoComplete="new-password" type="password" value={confirmPassword} onChange={e=>setConfirmPassword(e.target.value)}/></label></div>{password&&!passwordValid&&<p className={styles.warning}>{fa?"رمز عبور باید حداقل ۱۰ و حداکثر ۱۲۸ کاراکتر باشد.":"Password must be between 10 and 128 characters."}</p>}{confirmPassword&&!passwordsMatch&&<p className={styles.warning}>{fa?"تکرار رمز عبور یکسان نیست.":"Passwords do not match."}</p>}<label className={styles.remember}><input type="checkbox" checked={rememberMe} onChange={e=>setRememberMe(e.target.checked)}/><span>{fa?"ورود من روی این دستگاه حفظ شود":"Keep me signed in on this device"}</span></label><button disabled={busy||!registrationValid}>{busy?(fa?"در حال بررسی…":"Checking…"):mode==="bootstrap"?(fa?"ساخت حساب و رمز مالک":"Create owner account & password"):(fa?"احراز، ثبت‌نام و ساخت رمز":"Verify, register & set password")}</button></form>}</section>}

...
export default function ProfilePage() {
  ...
  async function invite() {
    setBusy(true); setError(""); setLastInviteUrl("");
    try {
      const result = await inviteTeamMember({
        firstName: inviteFirst, lastName: inviteLast,
        email: inviteEmail || undefined, mobile: inviteMobile || undefined,
      ...
      setInviteFirst(""); setInviteLast(""); setInviteEmail(""); setInviteMobile("");
  ...
  return (
    <main className={styles.page} dir={isRtl ? "rtl" : "ltr"}>
      <section className={styles.identity}>
        ...
        <div><span>GLYMIZE PROFILE</span><h1>{displayName}</h1><p>{isPhysician ? `${fa ? "پزشک · کد نظام پزشکی" : "Physician · Medical Council"}: ${user.medicalCouncilCode ?? "—"}` : `${fa ? "دستیار/پرستار" : "Assistant / nurse"} · ${user.practiceName}`}</p></div>
      </section>

      <section className={styles.card}>
        <div className={styles.heading}><div><h2>{fa ? "مشخصات و فضای کار" : "Profile & workspace"}</h2><p>{fa ? "نام، عکس و پوسته ترجیحی GLYMIZE روی حساب شما ذخیره می‌شود." : "Your name, photo, and preferred GLYMIZE workspace are stored on your account."}</p></div></div>
        <div className={styles.grid2}>
          ...
          <label><span>{fa ? "پوسته / Layout" : "Workspace layout"}</span><select value={layoutPreset} onChange={(e) => setLayoutPreset(e.target.value as LayoutPreset)}><option value="auto">Auto — Recommended</option><option value="command_center">Command Center</option><option value="focused_workflow">Focused Workflow</option><option value="compact_cards">Compact Cards</option></select></label>
          <label><span>{fa ? "عکس پروفایل" : "Profile photo"}</span><input type="file" accept="image/png,image/jpeg,image/webp" onChange={(e) => void uploadPhoto(e)} /><small>{fa ? "PNG/JPEG/WebP تا 350KB. در آینده عکس تأییدشده نظام پزشکی هم می‌تواند پیشنهاد شود." : "PNG/JPEG/WebP up to 350KB. A verified Medical Council photo may also be suggested later."}</small></label>
        </div>
        <div className={styles.actions}><button disabled={busy} onClick={() => void saveProfile()}>{fa ? "ذخیره پروفایل" : "Save profile"}</button>{photo && <button className={styles.secondary} disabled={busy} onClick={() => setPhoto(undefined)}>{fa ? "حذف عکس" : "Remove photo"}</button>}</div>
      </section>

      {isPhysician ? <>
        <section className={styles.card}>
          <div className={styles.heading}><div><h2>{fa ? "ایجاد دستیار / پرستار" : "Invite assistant / nurse"}</h2><p>{fa ? "هر دستیار حساب مستقل دارد و حتی وقتی پزشک از سیستم خارج است می‌تواند فقط در محدوده مجوزها کار کند." : "Each care-team member gets an independent account and can work within granted permissions even while the physician is signed out."}</p></div></div>
          ...
          <button disabled={busy || !inviteFirst.trim() || !inviteLast.trim() || (!inviteEmail.trim() && !inviteMobile.trim())} onClick={() => void invite()}>{fa ? "ساخت و ارسال دعوت" : "Create invitation"}</button>
          {lastInviteUrl && <div className={styles.inviteLink}><span>{fa ? "لینک دعوت" : "Invitation link"}</span><code>{lastInviteUrl}</code></div>}
        </section>

        <section className={styles.card}>
          <div className={styles.heading}><div><h2>{fa ? "تیم و سطح دسترسی" : "Care team & permissions"}</h2><p>{fa ? "تغییر مجوز یا غیرفعال‌کردن عضو، در درخواست بعدی Backend اعمال می‌شود." : "Permission or status changes are enforced by the backend on the next request."}</p></div></div>
      ...
      </> : <section className={styles.card}><h2>{fa ? "دسترسی‌های من" : "My access"}</h2><div className={styles.permissionGrid}>{PERMISSIONS.map((permission) => <div key={permission.key} data-active={user.permissions.includes(permission.key)}>{permission[locale]}</div>)}</div></section>}
